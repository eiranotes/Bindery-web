...
바인더리는 한국 독립 문구·일러스트 창작자가 행사 신청 마감, 참가 조건과
과거 회차의 변화를 같은 구조로 비교하고 준비하도록 돕는 행사 데이터베이스이자
운영 지식 아카이브입니다. Notes와 News는 행사 판단과 준비를 보조하고,
...
현재 행사·노트 예시는 `app/lib/data.ts`에 있습니다. 제품 검증 뒤에는 행사
마스터, 회차, 공식 출처와 확인 이력을 분리하고, 운영자 검수 없이 자동 수집
결과를 공개하지 않습니다. 비교와 회차 아카이브는 기존 컬렉션을 읽는 첫
구현이며, 다음 단계는 실제 행사 데이터 확보와 검증 상태 도입입니다.
